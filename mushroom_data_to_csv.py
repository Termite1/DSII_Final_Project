...
numCats = {0: 2,
            ...
            14: 9,
            15: 9,
            16: 2, #Technically 1, but putting 1 will give an error
            17: 4,
            18: 3, 
            19: 5,
            20: 9,
...
with open(file, 'r') as openfile:
    ...
    #Manually compute size of each category using the dictionary. Run through file, start vals at 0. 
    #Do proper for list to create. Create dictionary now? Count unique istances

    '''
    #runs through each line of data
    for line in openfile:
        ...
        for j in range(length):
            ...
            if current_line[j] in data_dictionary[j]: continue
            else:
    ...
    for line in openfile:        
        ...
        for j in range(length):
            ...
            #Should be redundant now with dictionary creation above
            else:
                data_dictionary[j].update({current_line[j]: len(data_dictionary[j])})
                if data_dictionary[j][current_line[j]] == 0: continue
                else: 
    ...
    out_file = open('mushroom_dataV2.csv', 'w+', newline='')
